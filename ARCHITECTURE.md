...
One field is unlike the rest: each locale's `guidance` is original content written in that locale's own language, recording the writing conventions that locale follows (form of address, tone, gendered forms, terminology the glossary doesn't cover). Because it isn't a translation of the English, the locale tooling skips it entirely — it is never machine translated and never counted unwritten — and it may be empty. It's shown and edited in localization mode, in the `Localizer` panel and at the top of the `/localize` workspace.
