...
In the IDE, [ProjectView](https://github.com/wordplaydev/wordplay/blob/main/src/components/project/ProjectView.svelte) wraps the `Evaluator`'s two internal modes (playing and stepping) in three creator-facing _evaluation modes_, persisted in the URL's `mode` param: ✏️ **edit** (sources, stage, and palette editable; the evaluator paused on a frozen frame; new stream inputs discarded), ⏸️ **step** (everything read-only; the debug timeline visible; inline values rendered; stage output selectable for inspection), and ▶️ **play** (everything read-only; streams live and the stage interactive). Edit and step both discard new stream inputs via the `Evaluator`'s `ignoringInputs` flag so stray interactions never extend the recorded input history; an unhandled exception while playing automatically switches to step mode at the exception frame.
...
Sound output is modeled symbolically, not as recordings: a [`Music`](https://github.com/wordplaydev/wordplay/blob/main/src/output/Music/Music.ts) output holds `Track`s of scale degrees played by `Instrument`s from a fixed palette, following the standard API pattern above (structure definition, wrapper class, converter, registered in `createDefaultShares`). Two structures carry static binds populated by the same `staticBuilder` mechanism `Color` uses for its color terms: `Music` exposes the named scales (`🎼.major`, `🎼.pentatonic`, …, defined in [scales.ts](https://github.com/wordplaydev/wordplay/blob/main/src/output/Music/scales.ts)) and `Instrument` exposes the palette (`🔈.piano`, `🔈.🥁`, …). Everything the player and visualizations know about an instrument — pitched vs. kit, kit-degree mapping, visualization hue — lives in [instruments.ts](https://github.com/wordplaydev/wordplay/blob/main/src/output/Music/instruments.ts), keyed by the instrument's opaque id; the pure degree-to-semitone math lives in [degrees.ts](https://github.com/wordplaydev/wordplay/blob/main/src/output/Music/degrees.ts). `Music` joins `Say` in `Stage`/`Group` content, collected by `getMusic()` mirroring `getSays()`. The full design, including the audio player and visual renderings built on this surface, is specified in [issue #390](https://github.com/wordplaydev/wordplay/issues/390#issuecomment-5149831130).
