...
import { decodeFlac, encodeMp3 } from './encode';
import {
    fetchCached,
    hashOf,
    readRemoteZipEntry,
...
export async function build(): Promise<string[]> {
    ...
    for (const instrument of Manifest) {
        ...
        for (const zone of zones) {
            ...
            // Percussion is measured by its loudest moment, sustained
            // instruments by integrated loudness — see measureShortTermLoudness.
            const measure =
                instrument.pitched === false
                    ? measureShortTermLoudness
                    : measureLoudness;
