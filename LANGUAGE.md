...
- **Reading.** A per-user setting toggles display between symbols (default) and words; in words mode each construct renders as its locale word, and conversely a word-typed construct renders as its symbol in symbols mode. This is render-only — the stored source is unchanged. A token rendered as something other than its source text — a keyword shown as a word, or a name shown in another language — is **atomic to the caret**: every caret position, click and selection boundary in it resolves to one of its two edges, because the editor's geometry indexes rendered text with source offsets and there is no honest mapping between two different strings. Clicking one puts the caret at its edge, which reverts it to the source text, which is what makes editing it coherent.
...
A language or region may be written by name as well as by code, so `'hola'/es`, `'hola'/Español`, and `'hola'/Spanish` are the same tag, as are `/es-MX`, `/es-México`, and `/es-Mexico`. The one region that is not an ISO 3166 assignment is Kosovo, written `/XK` or `/Kosovo` — the user-assigned code CLDR and BCP 47 use for it. Matching ignores case, accents, spaces, and punctuation — `/ESPANOL` and `/es` both name Spanish — and it never depends on the reader's own language, so a program means the same thing to everyone. A tag keeps the spelling it was written with; only its meaning is resolved, so `/Español` stays `/Español` in the source.
...
Some sequence a program's own output:
...
`Scene` shows a list of `Phrase`, `Group`, and `Shape` one at a time, and its value is whatever it is showing, so a scene can stand wherever an output can. What decides when it moves on is the output itself: an output with an `entering` pose or sequence is shown until every animation in it — including in a `Group`'s contents — has finished, and one without is shown for its `duration`. A `Boolean` may stand anywhere in the list as a **condition**: the scene waits there while it is `⊥` and walks on when it becomes `⊤`, which is how a scene advances on a click, a beat, or a guess rather than on a clock.

Seven optional inputs make a scene a transport. `loop` brings it back around to its first output after the last one finishes, and is read on every evaluation, so a loop can be ended as well as begun; a lap that waited for nothing — every output instant, no condition — does not come around, since it would otherwise re-evaluate the program as fast as the machine allows. `replay` starts it over from the first output and resets its lap count. `pause` holds it where it is, giving back whatever was left of the current output's `duration` when it is let go rather than starting that time over; a scene left paused never advances on its own. `go` sends it to a particular output, named either by position counting from 1 or by the `name` given to that output — the same name animations, `Choice`, and `Collision` know it by. `step` is the same instruction said as a distance rather than a place — `1` forward, `-1` back, from wherever the scene is — so a program that only ever moves one at a time never has to know which output is showing. And `name` names the scene itself, which is what a `Spotlight` filters on.

`until` is a condition standing after _every_ output, exactly as if it were written into the list after each one, which is how a scene waits for the same thing throughout: `Scene(lines until: Button())` is the whole of a story that advances on a click. It waits **after** an output has had its `duration` or finished animating rather than instead of it, and a `Boolean` written into the list is a separate wait that still applies, so the two compose. Unlike the others it is answered fresh on each evaluation, the way a condition in the list is — a scene never acts on the value it was last handed, or a momentary condition would still read as true when an output's time ran out.

`replay`, `go` and `step` want a **momentary** condition, one true only on the evaluation the event caused (`∆ Button()`), for the reason `Music`'s `replay` does: a lasting value would restart or pin the scene on every evaluation from then on. `pause` wants a **lasting** one. Positions count only the scene's outputs, never the conditions between them, so a three-slide show with a click between each is "1 of 3" rather than "1 of 5"; and `go` or `step` past either end stops there unless `loop` is `⊤`, in which case it comes around, which is what saves a program from range-checking every step forward and back.

`Spotlight` reports where a `Scene` is, as a `Slate` carrying the `scene`'s name, the `index` of the output showing, the `total` number of outputs, the `lap` count (0 the first time through), and whether the scene is `waiting` — held at a condition, or paused. Like `Downbeat` its value is never `ø`: before any scene has reported it holds an empty `Slate` whose `index` is 0, which a showing scene never has. Its optional `name` follows a single scene; with `ø` it follows whichever scene reports. A scene pushes to its spotlights and asks for one evaluation covering itself and all of them, so a spotlight and its scene can never disagree within a frame, and `∆` on either is true on a cut.
...
- Any other single output — a `Phrase`, `Group`, `Shape`, `Image`, `Say`, or `Music` — is placed on a default `Stage`.
...
An `Image` is a grid of colors drawn as one picture. Its `colors` are a list of rows, each a list of `Color`, and it paints one square per color; a row shorter than the widest leaves its tail unpainted. Unlike every other output its `description` is **required**, because a grid of colors has no words of its own for a reader who cannot see it to fall back on. Its size comes from `width` and `height`: giving one takes the other from the grid's shape, so the same subject drawn at more colors is the same size with smaller squares, and giving both breaks the shape on purpose. Two optional functions translate what is drawn, each given one color: `glyph` returns the text to draw in that square instead of painting it, and `recolor` returns the color to paint instead. Both are applied once, when the image is made, rather than while it is drawn.

An image is **one** output. It has one place, one pose and one body, so its squares do not animate, collide or become selectable individually — which is the trade that lets a picture of a thousand colors cost what one phrase costs, rather than a thousand.
...
A `Group` gives its content an `Arrangement`, and **an arrangement places its children**: it moves each one to where it decides, keeping its size and its shape. `Stack` decides how far down each child goes and aligns them across itself; `Row` decides how far along and aligns them down itself; `Grid` decides both. A `place` a program writes on a child overrides the axis an arrangement merely _aligns_ — a stack's x, a row's y — but never the axis it arranges along, since that is what a stack or a row is for. `Free` decides nothing, so there every child sits at its own `place`, exactly as it does on the stage.

A `Shape` is the exception, because it has no `place` input: the place it carries is built from its form's coordinates, which give it a size and a natural position rather than a request. So an arrangement places a shape like anything else, translating its geometry, and only the `Stage` and a `Free` group put it where its form says. To nudge any child a little from where an arrangement put it, give it an `offset`, which is applied after layout.

**Depth is never arranged.** No arrangement decides `z`, so a child's own depth stands in all four, and that is what lets output inside a group layer.
