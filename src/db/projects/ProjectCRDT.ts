...
export default class ProjectCRDT {
    ...
    /**
     * Initialize the CRDT from a list of plain-string source codes —
     * the bootstrap path when a project has no persisted `crdt`
     * snapshot yet (a fresh project, a v7-and-earlier migration, or a
     * test fixture written straight to Firestore).
     *
     * # Why we seed through a fixed-clientID intermediate
     *
     * The naive version inserts the source text on each peer's own
     * Y.Doc. Each Y.Doc has a random `clientID`, so the seed
     * operations get tagged with *different* IDs on different peers.
     * When two peers later exchange snapshots (e.g. one saves to
     * Firestore, the other folds), Yjs treats those as two distinct
     * insertions at position 0 and interleaves them — the merged
     * text contains the seed *twice*. Chromium's slightly looser
     * timing hid this in `toContainText` substring assertions;
     * Firefox surfaced it consistently.
     *
     * The fix: build the seed on a throwaway Y.Doc with `clientID = 0`
     * (any deterministic value would work; 0 is the simplest), encode
     * it to bytes, and apply those bytes to a freshly constructed
     * Y.Doc whose own clientID is random (used for *subsequent*
     * peer-specific edits). Every peer produces byte-identical seed
     * operations now, so Yjs's `(clientID, counter)` dedup makes the
     * cross-peer merge a no-op for the seed and only interleaves the
     * real per-peer edits.
     */
    static fromSources(codes: string[]): ProjectCRDT {
        const seed = new Y.Doc();
        // Override the autogenerated random clientID so the seed
        ...
        const crdt = new ProjectCRDT();
        Y.applyUpdateV2(crdt.doc, bytes);
        return crdt;
    }
