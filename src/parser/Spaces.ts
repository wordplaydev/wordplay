...
export default class Spaces {
    ...
    getAdditionalSpace(token: Token, preferredSpace: string): string {
        ...
        while(index < precedingSpace.length) {
            ...
            const lastLine = precedingSpace.substring(index + 1).indexOf("\n") < 0;
    ...
    withReplacement(replaced: Node, replacement: Node | undefined) {
        ...
        const replacementToken = replacedToken === undefined ? undefined : (replacement ? replacement.getFirstLeaf() : this.root.getNextToken(replacedToken, 1)) as Token | undefined;

        // Get the space prior to the replaced token.
        const space = replacedToken !== undefined ? this.#spaces.get(replacedToken) : undefined;
