...
export default class GalleryHowConcept extends Concept {
    ...
    getRepresentation(): Node {
        ...
        return (
            markup.getExamples()[0]?.program.expression ?? markup
        );
    }
    ...
    getNodes(): Set<Node> {
        return new Set(this.howTo.getText().map((text) => toMarkup(text)[0].getExamples()).flat());
    }
    ...
    isEqualTo(concept: Concept): boolean {
        return concept instanceof GalleryHowConcept && concept.howTo.getHowToId() === this.howTo.getHowToId();
    }
