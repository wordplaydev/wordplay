...
export default class Tree {
    ...
    getNearestAncestor<T extends Node>(type: Function): T | undefined {
        return (
            (this.getAncestors()?.find((n) => n instanceof type) as T) ??
            undefined
        );
    }
    ...
    inList() {
        return this.getContainingParentList() !== undefined;
    }
    ...
    getContainingParentList(before?: boolean): string | undefined {
        ...
        for (const name of parent.getChildNames()) {
            ...
            if (Array.isArray(field) && field.includes(this.node)) return name;
