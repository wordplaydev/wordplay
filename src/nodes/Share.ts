...
export default class Share extends Node implements Evaluable {
    ...
    computeConflicts(context: Context): Conflict[] {
        ...
        if(this.bind instanceof Bind && !this.bind.names.names.every(n => n.lang !== undefined))
    ...
    evaluate(evaluator: Evaluator) {
        ...
        const name = this.bind.names.names[0].getName();
        const value = name === undefined ? undefined : evaluator.resolve(name);
        if(value === undefined || name == undefined) 
            return new NameException(evaluator, this.bind.names.names[0].getName() ?? "");
        else
