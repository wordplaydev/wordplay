...
export default class Docs extends Node {
            ...
            .filter(
                (doc1) =>
                    this.docs.find(
                        (doc2) =>
                            doc1 !== doc2 &&
                            doc1.getLanguage() === doc2.getLanguage()
                    ) !== undefined
            )
            .map((doc) => doc.lang)
    ...
    getTranslations(): Translations {
        ...
        for (const docs of this.docs) {
            ...
            if (text.charAt(text.length - 1) === DOCS_SYMBOL)
    ...
    getTranslation(lang: LanguageCode[]): string {
        ...
        return preferredTranslation === undefined
            ? defaultTranslation
            : translations[preferredTranslation] ?? defaultTranslation;
    }
