...
function style(evaluator: Evaluator, font: string, size: number) {
    ...
    bindings.set(Style.inputs[0].names, new Text(evaluator.source.program, font));
    bindings.set(Style.inputs[1].names, new Measurement(evaluator.source.program, size, new Unit(undefined, [ new Dimension("pt")])));
...
function phrase(evaluator: Evaluator, text: string | Text, size: number=12, font: string="Noto Sans", ): Structure {
    const bindings = new Map<Names, Value>();
    bindings.set(Phrase.inputs[0].names, text instanceof Text ? text : new Text(evaluator.source.program, text));
...
function group(evaluator: Evaluator, ...phrases: Structure[]) {
    return createStructure(evaluator, Group as StructureDefinition, new Map().set(Group.inputs[1].names, new List(evaluator.source.program, phrases)));
}