...
abstract class Node {
    ...
    getWeight(): undefined | number { return undefined; }

    toString(): string { return `${this.getRichTextMarker()}${this.children.map(child => child.toString()).join("")}${this.getRichTextMarker()}`; }
    toHTML(): string { return `${this.getHTMLOpenMarker()}${this.children.map(child => child.toHTML()).join("")}${this.getHTMLCloseMarker()}`; }
...
class RichNode extends Node {
    ...
    getRichTextMarker() { return ""; }
    getHTMLOpenMarker() { return ""; }
    getHTMLCloseMarker() { return ""; }
...
class TextNode extends Node {
    ...
    gatherTextFormats(formats: Map<TextNode, CSSFormat>, parents: Node[]) {
        formats.set(this, { 
            ...
            weight: parents.find(p => p.getWeight() !== undefined)?.getWeight()
        });
    }
}
...
class UnderlineNode extends Node {
    ...
    getRichTextMarker() { return "_"; }
    getHTMLOpenMarker() { return "<u>"; }
    getHTMLCloseMarker() { return "</u>"; }
...
class LightNode extends Node {
    ...
    getRichTextMarker() { return "*"; }
    getHTMLOpenMarker() { return "<span class='light'>"; }
    getHTMLCloseMarker() { return "</span>"; }
    getWeight(): undefined | number { return 300; }
...
class BoldNode extends Node {
    ...
    getRichTextMarker() { return "**"; }
    getHTMLOpenMarker() { return "<strong>"; }
    getHTMLCloseMarker() { return "</strong>"; }
    getWeight(): undefined | number { return 500; }
...
class ExtraBoldNode extends Node {
    ...
    getRichTextMarker() { return "***"; }
    getHTMLOpenMarker() { return "<span class='extra'>"; }
    getHTMLCloseMarker() { return "</span>"; }
    getWeight(): undefined | number { return 700; }
...
class ItalicNode extends Node {
    ...
    getRichTextMarker() { return "/"; }
    getHTMLOpenMarker() { return "<em>"; }
    getHTMLCloseMarker() { return "</em>"; }
...
function parseNodes(symbols: string[], awaiting?: string): Node[] {
    ...
    while(symbols.length > 0 && (awaiting === undefined || !symbols.join("").startsWith(awaiting))) {
        ...
        if(next === "*") {
            ...
            let text = "";
            while(symbols.length > 0 && symbol !== "*" && symbol !== "/" && symbol !== "_") {
                text += symbols.shift();
                symbol = symbols[0];
            }
            if(text.length > 0)
