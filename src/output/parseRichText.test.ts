...
test('Test rich text parsing', () => {
    ...
    expect(parseRichText('hello /world/').toHTML()).toBe(
        'hello <em>world</em>'
    );

    expect(parseRichText('hello *world*').toString()).toBe('hello *world*');
    expect(parseRichText('hello *world*').toHTML()).toBe(
        "hello <span class='light'>world</span>"
    );
    ...
    expect(parseRichText('hello ***world***').toString()).toBe(
        'hello ***world***'
    );
    expect(parseRichText('hello ***world***').toHTML()).toBe(
        "hello <span class='extra'>world</span>"
    );

    expect(parseRichText('hello /**wor**ld/!').toString()).toBe(
        'hello /**wor**ld/!'
    );
    expect(parseRichText('hello /**wor**ld/!').toHTML()).toBe(
        'hello <em><strong>wor</strong>ld</em>!'
    );
});
