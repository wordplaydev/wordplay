...
export default class Sequence extends Output {
    ...
    getNextPose(current: Pose): Pose | undefined {
        ...
        // If it's not in this sequence, recursively check if a subsequence has it.
            ...
            for(const sequence of this.poses.filter(p => p instanceof Sequence) as Sequence[]) {
...
export function toSequence(value: Value | undefined) {
    ...
    if(!(value instanceof Structure && value.type === SequenceType)) return undefined;
    
    const count = toDecimal(value.resolve("count"));
    const poses = value.resolve("poses");
    if(!(poses instanceof List)) return undefined;
    const mapped = poses.values.map(val => toPose(val) ?? toSequence(val)) as (Pose | Sequence | undefined)[];
    if(mapped.some(val => val === undefined))
        return undefined;

    return count && poses instanceof List ? new Sequence(value, count.toNumber(), mapped as (Pose | Sequence)[]) : undefined;
