...
export default class Number extends Primitive {
    ...
    static fromToken(number: Token): Decimal {
        ...
        // Infinity
        if (number.isTokenType(TokenType.Infinity)) {
        ...
        else if (number.isTokenType(TokenType.Decimal)) {
        ...
        else if (number.isTokenType(TokenType.Base)) {
            return convertBase(text).times(negated ? -1 : 1);
        } else if (number.isTokenType(TokenType.RomanNumeral)) {
            return convertRoman(text).times(negated ? -1 : 1);
        } else if (number.isTokenType(TokenType.JapaneseNumeral)) {
            return convertJapanese(text).times(negated ? -1 : 1);
        }
        // If it matches the Pi token, convert to Pi.
        else if (number.isTokenType(TokenType.Pi)) {
