...
test('Test number translation', () => {
    ...
    expect(new Measurement(literal, -200302.123).toString()).toBe(
        '-200302.123'
    );
    ...
    expect(
        new Measurement(
            literal,
            new Token('18.2', TokenType.DECIMAL)
        ).toString()
    ).toBe('18.2');
    expect(
        new Measurement(
            literal,
            new Token('200302.123', TokenType.DECIMAL)
        ).toString()
    ).toBe('200302.123');
    expect(
        new Measurement(
            literal,
            new Token('0.000000001', TokenType.DECIMAL)
        ).toString()
    ).toBe('1e-9');
    expect(
        new Measurement(
            literal,
            new Token('001.100', TokenType.DECIMAL)
        ).toString()
    ).toBe('1.1');
    expect(
        new Measurement(
            literal,
            new Token('1000', TokenType.DECIMAL)
        ).toString()
    ).toBe('1000');
    expect(
        new Measurement(literal, new Token('∞', TokenType.INFINITY)).toString()
    ).toBe('Infinity');

    // Test Japanese numbers.
    expect(
        new Measurement(literal, new Token('十', TokenType.JAPANESE)).toString()
    ).toBe('10');
    expect(
        new Measurement(
            literal,
            new Token('二十', TokenType.JAPANESE)
        ).toString()
    ).toBe('20');
    expect(
        new Measurement(
            literal,
            new Token('二万', TokenType.JAPANESE)
        ).toString()
    ).toBe('20000');
    expect(
        new Measurement(
            literal,
            new Token('二十・二分', TokenType.JAPANESE)
        ).toString()
    ).toBe('20.2');
    expect(
        new Measurement(
    ...
    expect(
        new Measurement(
            literal,
            new Token('99万', TokenType.JAPANESE)
        ).toString()
    ).toBe('990000');

    // Test roman numerals.
    expect(
        new Measurement(literal, new Token('Ⅹ', TokenType.ROMAN)).toString()
    ).toBe('10');
    expect(
        new Measurement(literal, new Token('ⅩⅩ', TokenType.ROMAN)).toString()
    ).toBe('20');
    expect(
        new Measurement(literal, new Token('ⅩⅩⅩⅠⅩ', TokenType.ROMAN)).toString()
    ).toBe('39');
    expect(
        new Measurement(
            literal,
            new Token('ⅭⅭⅩⅬⅤⅠ', TokenType.ROMAN)
        ).toString()
    ).toBe('246');
    expect(
        new Measurement(
            literal,
            new Token('ⅮⅭⅭⅬⅩⅩⅩⅠⅩ', TokenType.ROMAN)
        ).toString()
    ).toBe('789');
    expect(
        new Measurement(
            literal,
            new Token('ⅯⅯⅭⅮⅩⅩⅠ', TokenType.ROMAN)
        ).toString()
    ).toBe('2421');
    ...
    expect(
        new Measurement(
            literal,
            new Token('2;10101.01', TokenType.BASE)
        ).toString()
    ).toBe('21.25');
});
...
test('Test equalities and inequalities', () => {
    ...
    expect(
        new Measurement(literal, 5).lessThan(
            literal,
            new Measurement(literal, 3)
        ).bool
    ).toBe(false);
    expect(
        new Measurement(literal, 5).lessThan(
            literal,
            new Measurement(literal, 10)
        ).bool
    ).toBe(true);
    expect(
        new Measurement(literal, 100.1).lessThan(
            literal,
            new Measurement(literal, 100.2)
        ).bool
    ).toBe(true);
    expect(
        new Measurement(literal, 100.1).lessThan(
    ...
    expect(
        new Measurement(literal, -1).lessThan(
            literal,
            new Measurement(literal, -100)
        ).bool
    ).toBe(false);
    expect(
        new Measurement(literal, -100).lessThan(
            literal,
            new Measurement(literal, -1)
        ).bool
    ).toBe(true);
    expect(
        new Measurement(literal, -1).greaterThan(
            literal,
            new Measurement(literal, -100)
        ).bool
    ).toBe(true);
    expect(
        new Measurement(literal, -100).greaterThan(
            literal,
            new Measurement(literal, -1)
        ).bool
    ).toBe(false);
...
test('Test addition', () => {
    expect(
        new Measurement(literal, 1)
            .add(literal, new Measurement(literal, 2))
            .toString()
    ).toBe('3');
    expect(
        new Measurement(literal, 1)
            .add(literal, new Measurement(literal, 10))
            .toString()
    ).toBe('11');
    expect(
        new Measurement(literal, -1)
            .add(literal, new Measurement(literal, -100))
            .toString()
    ).toBe('-101');
    expect(
        new Measurement(literal, 1.1)
            .add(literal, new Measurement(literal, 2.882))
            .toString()
    ).toBe('3.982');
    expect(
        new Measurement(literal, 1.1)
            .add(literal, new Measurement(literal, 2.982))
            .toNumber()
    ).toBe(4.082);
    expect(
        new Measurement(literal, -1.1)
    ...
    expect(
        new Measurement(literal, 2791)
            .add(literal, new Measurement(literal, -169.9))
            .toString()
    ).toBe('2621.1');
});
...
test('Test subtraction', () => {
    expect(
        new Measurement(literal, 1)
            .subtract(literal, new Measurement(literal, 2))
            .toString()
    ).toBe('-1');
    expect(
        new Measurement(literal, 1)
            .subtract(literal, new Measurement(literal, 10))
            .toString()
    ).toBe('-9');
    expect(
        new Measurement(literal, -1)
            .subtract(literal, new Measurement(literal, -100))
            .toString()
    ).toBe('99');
    expect(
        new Measurement(literal, 1.1)
            .subtract(literal, new Measurement(literal, 2.882))
            .toString()
    ).toBe('-1.782');
    expect(
        new Measurement(literal, 1.1)
            .subtract(literal, new Measurement(literal, 2.982))
            .toString()
    ).toBe('-1.882');
});
...
test('Test multiplication', () => {
    expect(
        new Measurement(literal, 11)
            .multiply(literal, new Measurement(literal, 12))
            .toString()
    ).toBe('132');
    expect(
        new Measurement(literal, -11)
    ...
    expect(
        new Measurement(literal, 0.11)
            .multiply(literal, new Measurement(literal, -0.12))
            .toString()
    ).toBe('-0.0132');
    expect(
        new Measurement(literal, 1)
            .multiply(literal, new Measurement(literal, 1))
            .toString()
    ).toBe('1');
    expect(
        new Measurement(literal, 1)
            .multiply(literal, new Measurement(literal, 0))
            .toString()
    ).toBe('0');
});
...
test('Test division', () => {
    expect(
        new Measurement(literal, 1)
            .divide(literal, new Measurement(literal, 1))
            .toString()
    ).toBe('1');
    expect(
        new Measurement(literal, 0)
            .divide(literal, new Measurement(literal, 1))
            .toString()
    ).toBe('0');
    expect(
        new Measurement(literal, 1)
            .divide(literal, new Measurement(literal, 0))
            .toString()
    ).toBe(NONE_SYMBOL);
    expect(
        new Measurement(literal, 10)
            .divide(literal, new Measurement(literal, 5))
            .toString()
    ).toBe('2');
    expect(
        new Measurement(literal, -10)
    ...
    expect(
        new Measurement(literal, 1)
            .divide(literal, new Measurement(literal, 3))
            .toString()
    ).toBe('0.33333333333333333333');
    expect(
        new Measurement(literal, 1)
            .divide(literal, new Measurement(literal, 2))
            .toString()
    ).toBe('0.5');
    expect(
        new Measurement(literal, 11)
    ...
    expect(
        new Measurement(literal, 2.1)
            .divide(literal, new Measurement(literal, 0.1))
            .toString()
    ).toBe('21');
    expect(
        new Measurement(literal, -2.1)
...
test('Test conversions', () => {
    ...
    // DISTANCE
    // Direct conversions
    expect(Evaluator.evaluateCode('1m→#pm')?.toString()).toBe(
        '1000000000000pm'
    );
    ...
    expect(Evaluator.evaluateCode('1kg→#oz')?.toString()).toBe('35.274oz');
    expect(Evaluator.evaluateCode('1000mg→#lb')?.toString()).toBe(
        '0.002204625lb'
    );
