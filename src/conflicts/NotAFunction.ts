...
export default class NotAFunction extends Conflict {
    ...
    constructor(evaluate: Evaluate | BinaryOperation | UnaryOperation, expression: Expression | Token, type: Type | undefined) {
        super(false);
    ...
    getExplanations(context: Context): Translations { 
        return {
            "😀": TRANSLATE,
            eng: this.type ? 
                `Couldn't find a function named ${this.expression instanceof PropertyReference ? `named "${this.expression.name?.toWordplay()}"` : this.expression.toWordplay()} on ${this.type.getDescriptions(context).eng}.` :
