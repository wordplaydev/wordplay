 ...
 * Raise a byte budget only for that, never to accommodate a new *module*
 * leaking in — the file counts and the runtime-reachability test below are
 * what guard the 2MB language runtime this file exists for.
 */
test.each([
    ['src/routes/+layout.svelte', 475, 3.36],
    ['src/components/app/Page.svelte', 495, 3.57],
