 ...
 *
 * Each surface is set up ONCE and scanned in both schemes, via
 * `expectNoAxeViolationsInBothSchemes`. That halves the browser contexts as well
 * as the navigations: every one of these signs in, and restoring Firebase Auth's
 * IndexedDB into a fresh context is not cheap. Measured on the project editor
 * with a running stage, a page flipped mid-test gives axe results identical to a
 * fresh navigation already in that scheme — see the helper.
 */
...
test.describe('authed views', () => {
    ...
    test(`a gallery's characters have no WCAG 2.2 AA violations`, async ({
        ...
        try {
            ...
            // Unique per run: a creator's character names have to be
            // unique, and a colliding one is deliberately not saved, which
...
test.describe('tutorial', () => {
    test(`tutorial lesson has no WCAG 2.2 AA violations`, async ({ page }) => {
